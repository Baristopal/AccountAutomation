...
public class CheckDal : ICheckDal
{
    ...
    {
        string query = "SELECT * FROM Checks WITH(NOLOCK) WHERE IsDeleted=0 AND CompanyId=@CompanyId";
    ...
    {
        return await _dbConnection.QuerySingleOrDefaultAsync<CheckModel>("SELECT * FROM Checks WHERE Id=@Id AND IsDeleted = 0", new { Id = id });
    }

    public async Task<decimal> GetChecksTotalAmount(string processType, int companyId)
    {
        string sql = "SELECT Price FROM Checks WITH(NOLOCK) WHERE IsDeleted = 0 AND ProcessType = @ProcessType AND CompanyId=@CompanyId";
